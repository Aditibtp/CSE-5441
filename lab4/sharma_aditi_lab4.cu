...
unsigned long long int *countGflop = 0;
unsigned long long int *d_countGflop = 0;
...
//function to calculate individual box dsv on GPU

__device__ void calculateDsvForBox(int box_index, Grid_box *grid_boxes, double *old_dsv, double *dsv_c, int total_boxes, double affect_rate, double epsilon, unsigned long long int* countGflop){
  ...
  if(box_peri > 0){
    ...
    atomicAdd(&countGflop[0],3);
  }
  dsv_c[cur] = cur_temp - offset;
}

/**************** KERNEL function ******************/
__global__ void Compute_dsv(Grid_box *grid_boxes, double *old_dsv, double *dsv_c, int total_boxes, double affect_rate, double epsilon, unsigned long long int* countGflop){
...
int main(int argc, char *argv[]){
  ...
  countGflop = (unsigned long long int*) malloc(sizeof(unsigned long long int));
  countGflop[0] = 1;
  ...
  if(cudaMalloc(&d_countGflop, sizeof(unsigned long long int)) != cudaSuccess){
  ...
  if(cudaMemcpy(d_countGflop, countGflop, sizeof(unsigned long long int), cudaMemcpyHostToDevice) != cudaSuccess){
  ...
  unsigned long long int gflopcounter = 0;
  ...
  while(1){
     ...
     // cudaDeviceSynchronize();
      ...
      if(cudaMemcpy(countGflop, d_countGflop, sizeof(unsigned long long int), cudaMemcpyDeviceToHost) != cudaSuccess){
  ...
  printf("Total Gflop operations:   %llu\n", gflopcounter);
  if((time_t_end - time_t_start) > 0){
    printf("Gflop/sec operations:   %llu\n", (gflopcounter/(time_t_end - time_t_start)));
  }
  printf("\n********************************************************************************\n");
